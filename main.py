...
GLOBAL_LOOP = None
GITHUB_REPO_URL = "https://github.com/LingYingNX/Infinite-Canvas-xiugai"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/LingYingNX/Infinite-Canvas-xiugai/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/LingYingNX/Infinite-Canvas-xiugai/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/LingYingNX/Infinite-Canvas-xiugai/main"
...
def check_update():
    """服务端检测 GitHub 远端版本（走系统代理，避免浏览器跨域/被墙）。"""
    current = current_app_version()
    # 保持独立线程，避免网络请求拖慢首屏更新提示
...
def mark_canvas_task_running(task_id: str) -> None:
    with CANVAS_TASK_LOCK:
        if task_id in CANVAS_TASKS:
            CANVAS_TASKS[task_id]["status"] = "running"
            CANVAS_TASKS[task_id]["updated_at"] = time.time()

def mark_canvas_task_succeeded(task_id: str, result) -> None:
    with CANVAS_TASK_LOCK:
...
def mark_canvas_task_failed(task_id: str, detail, status_code: int, upstream_task_id: str = None) -> None:
    updates = {
        "status": "failed",
        "error": str(detail),
    ...
    if upstream_task_id is not None:
        updates["upstream_task_id"] = upstream_task_id
    updates["updated_at"] = time.time()
    with CANVAS_TASK_LOCK:
        CANVAS_TASKS[task_id].update(updates)

def get_canvas_task_or_404(task_id: str, message: str):
    with CANVAS_TASK_LOCK:
...
async def run_canvas_image_task(task_id: str, payload: OnlineImageRequest):
    mark_canvas_task_running(task_id)
    try:
        result = await build_online_image_result(payload)
        mark_canvas_task_succeeded(task_id, result)
    except JimengPendingError as exc:
        ...
        mark_canvas_task_failed(task_id, detail, status_code, upstream_task_id=upstream_task_id)

@app.post("/api/canvas-image-tasks")
async def create_canvas_image_task(payload: OnlineImageRequest):
    task_id = f"canvas_img_{uuid.uuid4().hex}"
    create_canvas_task(task_id, "online-image", provider_id=payload.provider_id, model=payload.model)
...
async def get_canvas_image_task(task_id: str):
    return get_canvas_task_or_404(task_id, "画布任务不存在，可能服务已重启或任务已过期")
...
async def run_canvas_comfy_task(task_id: str, payload: GenerateRequest):
    ...
    try:
        ...
        detail = getattr(exc, "detail", None) or str(exc)
        status_code = getattr(exc, "status_code", 500)
        mark_canvas_task_failed(task_id, detail, status_code)
...
async def create_canvas_comfy_task(payload: GenerateRequest):
    ...
    create_canvas_task(task_id, "comfy", workflow_json=payload.workflow_json)
...
async def get_canvas_comfy_task(task_id: str):
    return get_canvas_task_or_404(task_id, "ComfyUI 任务不存在，可能服务已重启或任务已过期")
...
async def apimart_video_request_body(client, payload, provider, requested_model, is_veo31):
...
async def volcengine_video_request_body(client, payload):
...
def yuli_video_request_body(payload):
    yuli_images = []
...
def generic_video_request_body(payload):
    image_payload = []
...
async def post_canvas_video_json(client, provider, submit_urls, body):
    raw = None
    html_response = None
    last_response = None
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            # --- 构造图片载荷 ---
            if is_apimart:
                body = await apimart_video_request_body(client, payload, provider, requested_model, is_veo31)
            elif is_volcengine and not volc_is_proxy:
                body = await volcengine_video_request_body(client, payload)
            elif is_yuli:
                body = yuli_video_request_body(payload)
            else:
                body = generic_video_request_body(payload)
            raw, submit_url = await post_canvas_video_json(client, provider, submit_urls, body)
        ...
        canvas_video_status_error_hint(exc, provider, requested_model, payload)
    except httpx.HTTPError as exc:
...
async def import_canvas_file(
    ...
    try:
        if filename.endswith(".zip") or raw[:2] == b"PK":
            imported, resource_mapping, import_dir = import_canvas_zip(raw)
        else:
    ...
    try:
        ...
        if created_canvas_id:
            try:
                os.remove(canvas_path(created_canvas_id))
            except Exception:
                pass
        if import_dir:
            shutil.rmtree(import_dir, ignore_errors=True)
...
def import_canvas_zip(raw):
    ...
    try:
        ...
        return canvas_data, mapping, import_dir
    except Exception:
        if import_dir:
            shutil.rmtree(import_dir, ignore_errors=True)
        raise
